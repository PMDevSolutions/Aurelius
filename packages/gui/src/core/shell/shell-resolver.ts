...
export interface ShellResolver {
  ...
  resolveTool(name: "node" | "pnpm" | "claude" | (string & {})): Promise<string | null>;
}
...
export class DefaultShellResolver implements ShellResolver {
  ...
  async resolveBash(): Promise<string> {
    ...
    // AURELIUS_BASH is a GUI-development-only env var (documented in
    // docs/GUI.md); it is not part of any project configuration.
    const override = env.AURELIUS_BASH;
