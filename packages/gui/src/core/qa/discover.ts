 ...
 * losses to .aurelius/plans/<slug>/fidelity-report.md. Returns repo-relative
 * paths, sorted by slug.
 */
export async function listFidelityReports(repoRoot: string): Promise<string[]> {
  const plansDir = join(repoRoot, ".aurelius", "plans");
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (await exists(join(plansDir, entry.name, "fidelity-report.md"))) {
      reports.push(`.aurelius/plans/${entry.name}/fidelity-report.md`);
    }
  }
  return reports.sort();
}
